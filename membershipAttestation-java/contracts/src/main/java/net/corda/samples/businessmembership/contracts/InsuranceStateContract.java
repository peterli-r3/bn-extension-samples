...
public class InsuranceStateContract implements Contract {
    ...
    public String InsuranceStateContract_ID = "net.corda.samples.businessmembership.contracts.InsuranceStateContract";
    ...
    public void verify(@NotNull LedgerTransaction tx) throws IllegalArgumentException {
        ...
        if (command.equals(new Commands.Issue())){
    ...
    public void verifyMembershipsForMedInsuranceTransaction(LedgerTransaction tx, String networkId,
        ...
        requireThat(require -> {
            ...
            MembershipState insuranceMembership = membershipReferenceStates.stream()
                    .filter(it -> (it.getNetworkId() == networkId && it.getIdentity().getCordaIdentity() == insurance))
                    .collect(Collectors.toList()).get(0);
            MembershipState careProviderMembership = membershipReferenceStates.stream()
                    .filter(it -> (it.getNetworkId() == networkId && it.getIdentity().getCordaIdentity() == careProvider))
                    .collect(Collectors.toList()).get(0);
            require.using("Insurance "+ commandName+" transaction should have insurance's reference membership state", insuranceMembership!= null);
            require.using("Insurance "+ commandName+" transaction should have careProvider's reference membership state", careProviderMembership!= null);

            //Exam the customized Identity
            require.using("careProvider should be active member of Business Network with "+networkId, insuranceMembership.isActive());
